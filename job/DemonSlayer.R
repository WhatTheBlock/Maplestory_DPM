...
option <- factor(levels=BSkill)
value <- c()
info <- c(35 + 10 * ceiling(DemonSlayerBase$SkillLv/5), 120 - DemonSlayerBase$SkillLv, 990, T, T, T, T)
info <- data.frame(BInfo, info)
colnames(info) <- c("option", "value")
...
DemonSlayerBuff <- list(DF=DF, DFMaxForce=DFMaxForce, DemonBooster=DemonBooster, VampiricTouch=VampiricTouch, MapleSoldier=MapleSoldier, Metamorphosis=Metamorphosis, 
                        InfinityForce=InfinityForce, 
                        DevilCryDebuff=DevilCryDebuff, RemainTimeReinforce=RemainTimeReinforce, BlueBlood=BlueBlood, DemonicFortitude=DemonicFortitude, 
...
## Petbuff : DemonBooster(0ms), MapleSoldier(0ms), UsefulSharpEyes(900ms), UsefulCombatOrders(1500ms), (UsefulAdvancedBless)
if(sum(names(Useful)=="UsefulAdvancedBless") >= 1) {
...
info <- c(190, 2, 330, NA, 0, NA, NA, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
DemonSlash <- rbind(data.frame(option, value), info)

option <- factor(c("CRR", "BDR", "IGR", "FDR"), levels=ASkill)
value <- c(100, 110, IGRCalc(c(50, ifelse(GetCoreLv(DemonSlayerCore, "DemonSlash")>=40, 20, 0))), FDRCalc(c(30, 10, 2 * GetCoreLv(DemonSlayerCore, "DemonSlash"))))
info <- c(680, 3, 330, NA, 0, NA, NA, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
info <- c(515 + 5 * DemonSlayerSpec$SkillLv, 7, 840, NA, 14, T, T, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
value <- c(100, 50)
info <- c(975 + 39 * GetCoreLv(DemonSlayerCore, "Jormungand"), 12, 810, 1080, 135 - floor(GetCoreLv(DemonSlayerCore, "Jormungand")/2), T, F, F) ## StartATK : 1980ms, Hits : Vary by Level
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
option <- factor(c("CRR", "IGR"), levels=ASkill)
value <- c(100, 50)
info <- c(1050 + 42 * GetCoreLv(DemonSlayerCore, "Jormungand"), 15, 0, NA, 135 - floor(GetCoreLv(DemonSlayerCore, "Jormungand")/2), T, F, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
option <- factor(c("CRR", "IGR", "FDR"), levels=ASkill)
value <- c(50, 30, 90)
info <- c(250 + 11 * GetCoreLv(DemonSlayerCore, "DemonBane"), 6, 240, NA, 120, T, F, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
option <- factor(c("CRR", "IGR", "FDR"), levels=ASkill)
value <- c(50, 30, 90)
info <- c(625 + 25 * GetCoreLv(DemonSlayerCore, "DemonBane"), 7, 120, NA, 120, T, F, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
option <- factor(c("CRR", "IGR", "FDR"), levels=ASkill)
value <- c(50, 30, 90)
info <- c(625 + 25 * GetCoreLv(DemonSlayerCore, "DemonBane"), 7, 0, NA, 120, T, F, F)
info <- data.frame(AInfo, info)
colnames(info) <- c("option", "value")
...
DemonSlayerCycle <- function(DealCycle, ATKFinal, BuffFinal, SummonedFinal, Spec, SkipStructure, 
  ...
  DFCost <- data.frame(Skills=c("DemonImpact", "DevilCry", "Jormungand", "DemonBanePre", "VampiricTouch", "Metamorphosis", "InfinityForce", "BlueBlood", "DemonAwakening"), 
                       Cost=c(7, 24, 48, 48, 0, 32, 80, 0, 80))
  ...
  for(k in 2:length(BuffList)) {
    ...
    while(DealCycle$Time[nrow(DealCycle)] + DealCycle$Time[1] < BuffStartTime) {
      ...
      ## Devil Cry
      if(CryRemain==0 & DealCycle$DF[nrow(DealCycle)] >= subset(DFCost, DFCost$Skills=="DevilCry")$Cost & k!=length(BuffList) |
         CryRemain==0 & DealCycle$DF[nrow(DealCycle)] >= subset(DFCost, DFCost$Skills=="DevilCry")$Cost & k==length(BuffList) & DealCycle$Time[nrow(DealCycle)] + DealCycle$Time[1] < BuffStartTime - 5000) {
      ...
      ## Demon Bane & Demon Awakening
      ...
      else if(nrow(subset(DealCycle, DealCycle$Skills=="DemonBanePre")) >= 1 & nrow(subset(DealCycle, DealCycle$Skills=="DemonBanePre")) < 3 & 
              DealCycle$Time[nrow(DealCycle)] + DealCycle$Time[1] - max(subset(DealCycle, DealCycle$Skills=="DemonBanePre")$Time) >= ATKFinal[rownames(ATKFinal)=="DemonBanePre", ]$CoolTime * 1000) {
        ## Demon Slash - for RemainTime Reinforce
        DealCycle <- DCATK(DealCycle, "DemonSlash", ATKFinal)
        DealCycle <- DFBBLogic(DealCycle, DFAbsorb, DFCost)
        CryRemain <- max(0, CryRemain - DealCycle$Time[1])
      ...
      else if(sum(nrow(subset(DealCycle, DealCycle$Skills=="DemonAwakening")) == c(1, 3, 5)) >= 1 & 
        ...
        if(DealCycle$DF[nrow(DealCycle)] < 80) {
      ...
      ## Cerberus
      else if(CBRemain==0 & DealCycle$DemonAwakening[nrow(DealCycle)] == 0 & DealCycle$DF[nrow(DealCycle)] < 50 |
              CBRemain==0 & DealCycle$DemonAwakening[nrow(DealCycle)] == 0 & sum("InfinityForce" == BuffList[[k]]) > 0 & DealCycle$Time[nrow(DealCycle)] + DealCycle$Time[1] > BuffStartTime - 15000 & 
              DealCycle$Time[nrow(DealCycle)] + DealCycle$Time[1] < BuffStartTime - 5000) {
        DealCycle <- DCATK(DealCycle, "Cerberus", ATKFinal)
        DealCycle <- DFBBLogic(DealCycle, DFAbsorb, DFCost)
        ONRemain <- max(0, ONRemain - DealCycle$Time[1])
        OGRemain <- max(0, OGRemain - DealCycle$Time[1])
    ...
    if(k != length(BuffList)) {
      for(i in 1:length(BuffList[[k]])) {
        if(sum(rownames(BuffFinal)==BuffList[[k]][i]) > 0) {
          if(BuffList[[k]][i]=="OrthrosBuff") {
          ...
          } else if(BuffList[[k]][i]=="Metamorphosis") {
            DealCycle <- DCSummoned(DealCycle, "MetamorphosisATK", SummonedFinal)
            DealCycle <- DFBBLogic(DealCycle, DFAbsorb, DFCost)
            ONRemain <- max(0, ONRemain - DealCycle$Time[1])
            OGRemain <- max(0, OGRemain - DealCycle$Time[1])
            ...
            CBAWRemain <- max(0, CBAWRemain - DealCycle$Time[1])
            CBRemain <- max(0, CBRemain - DealCycle$Time[1])
          } else if(BuffList[[k]][i]=="InfinityForce") {
            DealCycle <- DCATK(DealCycle, "Cerberus", ATKFinal)
            DealCycle <- DFBBLogic(DealCycle, DFAbsorb, DFCost)
            ONRemain <- max(0, ONRemain - DealCycle$Time[1])
            OGRemain <- max(0, OGRemain - DealCycle$Time[1])
            ...
            CBAWRemain <- max(0, CBAWRemain - DealCycle$Time[1])
            CBRemain <- CBCool - DealCycle$Time[1]
          } else if(BuffList[[k]][i]=="CallMastema") {
            DealCycle <- DCATK(DealCycle, "MastemaClaw", ATKFinal)
            DealCycle <- DFBBLogic(DealCycle, DFAbsorb, DFCost)
            ONRemain <- max(0, ONRemain - DealCycle$Time[1])
            OGRemain <- max(0, OGRemain - DealCycle$Time[1])
...
DemonSlayerDealCycle2 <- DemonSlayerCycle(DealCycle=DemonSlayerDealCycle2, 
                                          ...
                                          Spec=DemonSlayerSpec, 
                                          SkipStructure=DemonSlayerSkipATK,
                                          Period=240, CycleTime=720, AwakeningOffSlash=F)
