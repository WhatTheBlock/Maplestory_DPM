...
info <- c(70 * (100 + MechanicBase$MonsterLife$SummonedDuration + MechanicPassive$SummonDuration + MechanicBase$UnionChrs$SummonedDuration) / 100, 60, 0, F, T, T, F)
info <- data.frame(BInfo, info)
colnames(info) <- c("option", "value")
...
option <- factor("BDR", levels=BSkill)
value <- c((6 + ceiling(MechanicBase$PSkillLv/5)))
info <- c(70 * (100 + MechanicPassive$SummonDuration) / 100 + 10, 80 * (100 - MechanicBase$UnionChrs$CoolReduceP - 25) / 100 - MechanicBase$CoolReduce, 630, F, F, F, F)
info <- data.frame(BInfo, info)
colnames(info) <- c("option", "value")
