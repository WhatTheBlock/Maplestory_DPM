...
AddATKsCyclePalladin <- function(DealCycle) {
  for(i in 1:nrow(DealCycle)) {
    if(DealCycle$Skills[i]=="DivineStigma" | DealCycle$Skills[i]=="Blast" | DealCycle$Skills[i]=="Sanctuary" |
...
FPAddATKCycle <- function(DealCycle, ATKSkillList) {
  ## Poison Region FDR Decrease
  DC1 <- subset(DealCycle, DealCycle$Skills=="PoisonRegion")
  FDRDecrease <- c()
  for(i in 2:nrow(DC1)) {
    if(DC1$Time[i] - DC1$Time[i-1] < 400) {
      FDRDecrease <- c(FDRDecrease, as.numeric(rownames(DC1[i, ])))
    }
  }
  
  if(length(FDRDecrease) > 0) {
    DealCycle[FDRDecrease, ]$Skills <- "PoisonRegionAdd"
  }
  ...
  ## Megiddo Flame
  for(i in 1:nrow(DealCycle)) {
    if(DealCycle$Skills[i]==c("MegiddoFlame")) {
      DealCycle <- rbind(DealCycle, DealCycle[i, ])
      DealCycle$Skills[nrow(DealCycle)] <- c("MegiddoFlameAdd")
      for(j in 1:11) {
        DealCycle <- rbind(DealCycle, DealCycle[i, ])
        DealCycle$Skills[nrow(DealCycle)] <- c("Ignite")
        DealCycle <- rbind(DealCycle, DealCycle[i, ])
  ...
  ## Meteor Final Attack
  PosionNovaATKTimes <- 3 + ATKFinal[rownames(ATKFinal) == "PoisonNovaRemain", ]$AttackTimes / 12
  for(i in 1:nrow(DealCycle)) {
    if(sum(DealCycle$Skills[i]==c("FlameSweep", "MistEruption", "FlameHaze", "PoisonNovaStart"))==1) {
      DealCycle <- rbind(DealCycle, DealCycle[i, ])
      DealCycle$Skills[nrow(DealCycle)] <- c("MeteorFinalAttack")
    } else if(DealCycle$Skills[i]==c("PoisonNova123")) {
      for(j in 1:PosionNovaATKTimes) {
        DealCycle <- rbind(DealCycle, DealCycle[i, ])
        DealCycle$Skills[nrow(DealCycle)] <- c("MeteorFinalAttack")
      }
    }
  ...
  ## Ignite
  for(i in 1:nrow(DealCycle)) {
    if(sum(DealCycle$Skills[i]==c("FlameSweep", "Meteor", "FlameHaze", "Ifrit", "FuryofIfrit"))==1) {
      DealCycle <- rbind(DealCycle, DealCycle[i, ])
      DealCycle$Skills[nrow(DealCycle)] <- c("Ignite")
  ...
  for(i in 1:length(PotList)) {
    ...
    for(j in 1:nrow(PotList[[i]])) {
      ...
      if(nrow(UnionList) == 1) {
        ...
        NewDeal_1[j] <- sum(na.omit(EunwolYakjeomGanpa(DealCycle, ATKSkillsList, BuffList, SummonedSkillsList, NewSpec_1, Cores, NotBuffCols, NotBuffColOption)))
        ...
        for(k in 1:nrow(UnionList)) {
          ...
          NewDeal_2[k] <- sum(na.omit(EunwolYakjeomGanpa(DealCycle, ATKSkillsList, BuffList, SummonedSkillsList, NewSpec_2, Cores, NotBuffCols, NotBuffColOption)))
          if(k > 1) {
  ...
  for(i in 1:length(PotList)) {
    ...
    for(j in 1:nrow(PotList[[i]])) {
      ...
      if(nrow(UnionList) == 1) {
        NewSpec_1$BDR <- NewSpec_1$BDR + UnionList$BDR
        NewSpec_1$IGR <- IGRCalc(c(NewSpec_1$IGR, UnionList$IGR))
        NewDeal_1[j] <- sum(na.omit(XenonDealCalc(DealCycle, ATKSkillsList, BuffList, SummonedSkillsList, NewSpec_1, Cores, Collapse=T)))
        Union[j, ] <- c(UnionList$BDR, UnionList$IGR)
      } else {
        for(k in 1:nrow(UnionList)) {
          NewSpec_2 <- NewSpec_1
          NewSpec_2$BDR <- NewSpec_1$BDR + UnionList$BDR[k]
          NewSpec_2$IGR <- IGRCalc(c(NewSpec_1$IGR, UnionList$IGR[k]))
          NewDeal_2[k] <- sum(na.omit(XenonDealCalc(DealCycle, ATKSkillsList, BuffList, SummonedSkillsList, NewSpec_2, Cores, Collapse=T)))
          if(k > 1) {
            if(NewDeal_2[k] <= NewDeal_2[k - 1] & k < nrow(UnionList)) {
              NewDeal_1[j] <- NewDeal_2[k - 1]
              Union[j, ] <- c(UnionList$BDR[k - 1], UnionList$IGR[k - 1])
...
SpecMeanXenon <- function(JobName, DealCycle, DealData, ATKSkillsList, BuffList, SummonedSkillsList, Specs, Cores=(detectCores(logical=F)-1)) {
  ## Act Skills and Summoned Skills Collapse
  ATKSkills <- c(rownames(ATKSkillsList), rownames(SummonedSkillsList))
  ATKSkillsList <- rbind(ATKSkillsList, SummonedSkillsList[, c(-16, -18)])
  ATKSkillsList <- subset(ATKSkillsList, ATKSkillsList$Damage > 0)
  ...
  SpecCalcEach <- function(i) {
    ...
    if(nrow(Skills) == 0) {
      ...
      SpecEach$MainStat <- SpecEach$MainStat + SpecEach$SubStat1 + SpecEach$SubStat2 + 
        floor(Specs$MainStatWithoutStatP * DealCycle$SurplusSupplyStack[i] / 100) + 
        floor(Specs$SubStat1WithoutStatP * DealCycle$SurplusSupplyStack[i] / 100) + 
        floor(Specs$SubStat2WithoutStatP * DealCycle$SurplusSupplyStack[i] / 100)
      SpecEach$SubStat1 <- 0
      SpecEach$SubStat2 <- 0
...
EvanOptimization1 <- function(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, Specs, UnionBDRIGR, Cores=(detectCores(logical=F)-1), 
                              NotBuffCols=c(), NotBuffColOption=c()) {
  BDR <- min(40, UnionBDRIGR) : max(0, (UnionBDRIGR - 40))
  IGR <- UnionBDRIGR - BDR
  UnionList <- data.frame(BDR, IGR)
  ...
  for(i in 1:length(PotList)) {
    ...
    for(j in 1:nrow(PotList[[i]])) {
      ...
      if(nrow(UnionList) == 1) {
        NewSpec_1$BDR <- NewSpec_1$BDR + UnionList$BDR
        NewSpec_1$IGR <- IGRCalc(c(NewSpec_1$IGR, UnionList$IGR))
        NewDeal_1[j] <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, NewSpec_1, Cores, NotBuffCols, NotBuffColOption)))
        Union[j, ] <- c(UnionList$BDR, UnionList$IGR)
      } else {
        for(k in 1:nrow(UnionList)) {
          NewSpec_2 <- NewSpec_1
          NewSpec_2$BDR <- NewSpec_1$BDR + UnionList$BDR[k]
          NewSpec_2$IGR <- IGRCalc(c(NewSpec_1$IGR, UnionList$IGR[k]))
          NewDeal_2[k] <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, NewSpec_2, Cores, NotBuffCols, NotBuffColOption)))
          if(k > 1) {
            if(NewDeal_2[k] <= NewDeal_2[k - 1] & k < nrow(UnionList)) {
              NewDeal_1[j] <- NewDeal_2[k - 1]
              Union[j, ] <- c(UnionList$BDR[k - 1], UnionList$IGR[k - 1])
...
EvanOptimization2 <- function(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, Specs, HyperBase, ChrLv, CRROver, HyperStanceLv=0, Cores=(detectCores(logical=F)-1), 
                              NotBuffCols=c(), NotBuffColOption=c()) {
  lvs <- c()
  for(i in 1:9) {
    p <- 0
  ...
  while(remainpts > min(nextlevelpts)) {
    ...
    names(NewSpecs) <- c("Pre", colnames(HyperBase))
    PreDeal <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, NewSpecs[[i]], Cores, 
                                   NotBuffCols, NotBuffColOption)))
    NewSpecs$MainStat$MainStat <- NewSpecs$MainStat$MainStat + ifelse(nextlevelpts[1] > remainpts, 0, HyperStats$MainStat[lvs[1]+1])
    NewSpecs$SubStat1$SubStat1 <- NewSpecs$SubStat1$SubStat1 + ifelse(nextlevelpts[2] > remainpts, 0, HyperStats$SubStat1[lvs[2]+1])
    if(is.null(NewSpecs$SubStat2$SubStat2)==F) {
    ...
    NewDeal <- c()
    for(i in c(1:6, 8:9)) {
      NewDeal[i] <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, NewSpecs[[i+1]], Cores, 
                                        NotBuffCols, NotBuffColOption)))
    }
    if(CRROver==T) {
      BuffListNew <- BuffList
      CRIlv <- BuffList[rownames(BuffList)=="CriticalReinforce", ]$CDMR / NewSpecs$Pre$CRR * 100 - 20
      BuffListNew[rownames(BuffListNew)=="CriticalReinforce", ]$CDMR <- NewSpecs$CRR$CRR * ((CRIlv + 20) / 100)
      NewDeal[7] <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffListNew, SummonedSkillsList, NewSpecs[[8]], Cores, 
                                        NotBuffCols, NotBuffColOption)))
    } else {
      NewDeal[7] <- sum(na.omit(EvanBoW(DealCycle, DealCycle2, ATKSkillsList, BuffList, SummonedSkillsList, NewSpecs[[8]], Cores, 
                                        NotBuffCols, NotBuffColOption)))
    }
